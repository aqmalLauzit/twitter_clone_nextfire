...
export default function Post({ post,id }) {
  ...
  const likePost = async() => {
    if(session) {
      if(hasLiked){
        ...
        //menambahkan relasi satu postingan di likes oleh multiple user
  ...
  const deletePost = () => {
    if(window.confirm("Yakin akan menghapus postingan?")) {
      //delete document dari koneksi db, collection posts, where posts.id = id
  ...
  return (
    <div className="flex p-3 cursor-pointer border-b border-gray-200">
      ...
      <div className="flex items-center select-none">
          ...
          <HeartIconFilled onClick={likePost} className=" text-red-600 bg-red-100 h-9 w-9 hoverEffect p-2 hover:text-red-100 hover:bg-black" />
          ) : (
